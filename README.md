...
The classical Kelly formula, `f* = p - q/b`, has a closed form because a binary bet has exactly two outcomes. A vertical credit spread doesn't — its P&L is piecewise-linear in the underlying's terminal price, with a capped gain, a capped loss, and a full continuum in between. That breaks the closed-form case: sizing it correctly requires simulating the outcome distribution and solving numerically for the fraction of capital that maximizes long-run compound growth.
